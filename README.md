...
The notes column also marks folders whose name says their app or tool rebuilds them ("Rebuilt by its app", with the same rules as Free up space › Find more) and folders where nothing changed for six months or more ("No changes for 2 years"); the details pane shows the last change inside the selected folder. These are hints, not verdicts.

Measuring is read-only: the map reads names, sizes, modification dates and flags and never opens file contents. It reports what it could not measure instead of guessing:
...
**Signing status:** the beta is ad-hoc signed, **not Apple-notarized**; the project ships as open source on GitHub. The first time you open a downloaded build, macOS says it cannot check it. Open it once from **System Settings › Privacy & Security**: scroll down to the message about Avakasha and click **Open Anyway** (on macOS 14 and earlier, Control-click the app and choose Open also works). After that it opens normally. Do not turn off Gatekeeper. If you prefer not to, build it from source with one command (see [distribution instructions](docs/DISTRIBUTION.md)); the build script can also sign and notarize for anyone with a Developer ID.
...
This is beta 19. Code compilation and automated tests are not substitutes for testing on every macOS version. The Intel slice is cross-built; real Intel-machine verification and clean-Mac Gatekeeper verification are release checklist items, not implied claims.
